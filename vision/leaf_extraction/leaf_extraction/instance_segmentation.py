...
class ImageProcessor(Node):
    ...
    def point_cloud_callback(self, msg):
        ...
        self.masks_xyzs = self.extract_masks_xyzs(self.ordered_masks, self.points)

        # self.plotting_o3d()
        self.plot_masked_points_with_centers(5)
    ...
    def extract_masks(self):
        ...
        results = self.model([open_cv_image], imgsz=1280, save=True, conf=self.conf_cutoff)
        ...
        for result in results:
            ...
            for i in range(result.__len__()):  
                ...
                resized_mask = cv2.resize(mask_i, (width, height), interpolation=cv2.INTER_NEAREST)
    ...
    def extract_masks_xyzs(self, masks, points):
        masks_xyzs = []
        xyz_reshaped = points.reshape((720, 1280, 3))

        for mask in masks:
            mask_boolean = mask.astype(bool)
            masked_points = xyz_reshaped[mask_boolean]
            masks_xyzs.append(masked_points)

        return masks_xyzs

    def plot_masked_points_with_centers(self, top_n):
        num_masks = min(len(self.masks_xyzs), top_n)
        fig = plt.figure()
